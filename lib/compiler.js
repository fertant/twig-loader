...
module.exports = function(options) {
    return function(id, tokens, pathToTwig) {
        ...
        var processGeneralDependency = function(token) {
            if (token.value && token.value.startsWith('@')) {
                if (options.twigOptions && options.twigOptions.namespaces) {
                    var namespaces = options.twigOptions.namespaces;
                    Object.keys(namespaces).forEach( ns => {
                        discoverTwig(namespaces[ns]);
                    });
                }
                processPathAlias(token)
                token.value = hashGenerator(path.resolve(path.dirname(resourcePath), '/Users/Andrii_Shutov/Git/otsk/otsk-docksal/theme-repos/theme-maintenaastuf/themes/custom/otsuka_ds3_abilifyasimhcc/components/02-molecules/cards/card-b/card-b.twig'));    
            }
        };
